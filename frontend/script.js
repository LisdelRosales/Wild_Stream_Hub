...
async function handleFileUpload(event) {
    ...
    if (!selectedList) {
        showMessage('Please select a stream list first', 'error');
        return;
    }
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        try {
            ...
            const response = await fetch(`${API_URL}/stream-lists/${selectedList}/upload`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${accessToken}`
                },
            ...
            if (!data.success) {
                showMessage(`Failed to upload ${file.name}: ${data.message}`, 'error');
            }
        } catch (error) {
    ...
    handleStreamListChange(); // Reload video list
}
